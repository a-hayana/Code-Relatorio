...
data_tramitacao <- "01/01/2022"
fonte <- "Fonte: Portal de Informações Gerenciais em 01/01/2022 e Relatório de Atividades 2021."
...
full_especies <- readRDS("../data_raw/full_especies.rds")
...
text_a3 <- tabela_acervo |> 
  select(ano,recursal) |> 
  filter(ano %in% ano_atual) |> 
  mutate(acervo_rec4 = if_else(num_acervo4 < 0, "uma redução de magnitude de" , "um aumento de de magnitude de")) |> 
...
DT::datatable(t_1, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
...
DT::datatable(t_8, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
Historicamente há uma certa estabilidade em relação aos processos originários. Para os recursais, no entanto, há `r text_a3[4]` em relação a 2016 e de `r abs(num_acervo3)`% com relação a `r ano_anterior`.
...
DT::datatable(t_9, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
`r text_ramo`^[Há 12 processos sem cadastro de ramo do direito, por isso o total contabiliza 12 processos a menos.]
...
t_10 <- tabela_acervo |> 
  select(1,4:5) |> 
  mutate(eletronico = paste0(round(eletronico,3)*100,"%"),
         fisico = paste0(round(fisico,3)*100,"%"))

DT::datatable(t_10, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
DT::datatable(t_2, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_12 <- tab_percent_total |> 
  mutate(`Registrados à Presidência` = paste0(round(`Registrados à Presidência`,3),"%"),
        `Distribuídos aos Ministros` = paste0(round(`Distribuídos aos Ministros`,3),"%") )

DT::datatable(t_12, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_13 <- qtd_total_pres_min  

DT::datatable(t_13, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_3 <- qtd_recursal_pres_min |> 
   mutate(`Registrados à Presidência` = paste0(round(`Registrados à Presidência`,3),"%"),
        `Distribuídos aos Ministros` = paste0(round(`Distribuídos aos Ministros`,3),"%") )

DT::datatable(t_3, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_4 <- qtd_recursal_pres_min_2

DT::datatable(t_4, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_6 <- tabela_rec_total |> 
  select(1,5:7)

DT::datatable(t_6, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_6 <- data.frame(Processos = factor(df_classes$class_names), 
                       Proporção = df_classes$total_perc)

t_6 <- t_6 |> 
  select(1:2) |> 
  mutate(`Proporção` = paste0(round(`Proporção`,3),"%"))
...
t_5 <- names_provimento |> 
  select(1:3) |> 
   mutate(`Taxa de provimento` = paste0(`Taxa de provimento`,"%"),
        `Taxa de não provimento` = paste0(`Taxa de não provimento`,"%"))
  

DT::datatable(t_5, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
DT::datatable(t_7, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_7 <- dados |> 
  select(1:2) |> 
  mutate(`Proporção` = paste0(`Proporção`, "%"))
...
t_15 <- names_graph_decisoes |> 
  select(1:3)

DT::datatable(t_15, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
# Texto para nota -> Colocando a qtd de "decisão" que mesclou ao "decisão interlocutória"

text_nota <- full_especies |> 
  filter(subgrupo_andamento_comissao %in% "Decisão")

```

### {.tabset}

Além da decisão final, há outras espécies em menor quantidade, conforme apresentado na tabela seguinte com os respectivos quantitativos.^[Algumas *"Decisões"* receberam somente a classificação geral de "*Decisão*" (`r as.character(text_nota[[2]])`) e foram classificadas como *"Decisão Interlocutória"* para este relatório.]
...
t_17 <- structure(list(Ano = tabela_dec$ano, 
Processos = c(tabela_dec$decisoes_monocraticas)), 
            class = "data.frame", row.names = c(NA, -6L))

DT::datatable(t_17, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_19 <- structure(list(Ano = tabela_dec$ano, 
Processos = c(tabela_dec$decisoes_colegiadas)), 
            class = "data.frame", row.names = c(NA, -6L))

DT::datatable(t_19, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_20 <- tx_recorr |> 
  mutate(taxa_rec = paste0(taxa_rec,"%"))

DT::datatable(t_20, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_21 <- tabela_rec_baixa_total |> 
  select(1:3,5)

DT::datatable(t_21, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
...
t_22 <- tab_rg_final_1[1:2,]


DT::datatable(t_22, extensions = 'Buttons', options = list(
  dom = 'Bfrtip',
  buttons = c('copy', 'csv', 'excel', 'pdf', 'print','colvis'), pageLength = 80
)
)
